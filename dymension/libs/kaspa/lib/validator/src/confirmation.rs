...
pub async fn validate_confirmed_withdrawals(
    ...
    let anchor_kaspa_outpoint = TransactionOutpoint {
        transaction_id: KaspaHash::from_bytes(
            anchor_utxo.transaction_id.as_slice().try_into().map_err(
                |_| eyre::eyre!("Validator: Invalid anchor outpoint transaction ID length"),
            )?
        ),
    ...
    let new_kaspa_outpoint = TransactionOutpoint {
        transaction_id: KaspaHash::from_bytes(
            new_utxo.transaction_id.as_slice().try_into().map_err(
                |_| eyre::eyre!("Validator: Invalid new outpoint transaction ID length"),
            )?
        ),
    ...
    if outpoints.len() < 2 {
        return Err(eyre::eyre!("Validator: Insufficient outpoints in cache for validation"));
    }
    if outpoints[0] != anchor_kaspa_outpoint {
        return Err(eyre::eyre!("Validator: Anchor outpoint mismatch in cache"));
    }
    if outpoints[outpoints.len() - 1] != new_kaspa_outpoint {
        return Err(eyre::eyre!("Validator: New outpoint mismatch in cache"));
    }
    ...
    if !validate_message_ids_match(fxg, &collected_message_ids)? {
        warn!("Validator: Message IDs do not match progress indication");
        return Ok(false);
    }
