...
const ESCROW_ADDRESS: &'static str = "kaspatest:qzwyrgapjnhtjqkxdrmp7fpm3yddw296v2ajv9nmgmw5k3z0r38guevxyk7j0";
...
async fn validate_transaction(deposits: Vec<DepositFXG>) -> Result<Vec<bool>,Box<dyn Error>> {
  ...
  // Select encoding method to use, depending on node settings
    ...
    for deposit in &deposits {
        ...
        let RpcBlock{header,transactions,verbose_data} = client.get_block(block_hash,true).await?;

        // find tx in block
        let tx_index = verbose_data.ok_or("block data not found")?.transaction_ids.iter().position(|id|id == &tx_hash).ok_or("transaction not found in block")?;

        // get utxo in the tx from index in deposit
        let utxo: &kaspa_rpc_core::RpcTransactionOutput = transactions[tx_index].outputs.get(deposit.utxo_index).ok_or("utxo not found by index")?;
