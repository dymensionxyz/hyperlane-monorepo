...
mod tests {
    ...
    fn message_ids_from_payload() {
        ...
        let message_ids = crate::bridge::payload::MessageIDs::new(vec![
            crate::bridge::payload::MessageID(test_id),
        ]);

        // Convert to bytes and back
        let bytes = message_ids.to_bytes();
        let decoded = crate::bridge::payload::MessageIDs::from_bytes(bytes).unwrap();
